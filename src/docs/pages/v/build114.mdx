...
小结
------
我一共尝试了两种打包方式，一种是直接从雷鸟的release里下载的源码包。一种是从Debian的salsa里克隆的特化源码（而这个是不带测试脚本的）

我根据前者编写好了补丁文件，然后尝试编译打包，并取得了成功（能收发邮件+能运行测试），但没有多语言支持。于是转向尝试从Debian的salsa里克隆带有Debian特化的打包规则脚本的thunderbird。在修改了一些内容以后，运行里面的打包脚本。结果出来了多语言支持包，但是却没有thunderbird本体。至于为什么会这样，我还得继续研究。
...
* mozconfig
```diff
--- /dev/null   2023-06-16 12:11:42.172000000 +0800
+++ mozconfig   2023-06-30 20:20:09.548008676 +0800
@@ -0,0 +1,5 @@
...
\ 文件尾没有换行符
diff --color -Nru a/third_party/rust/cty/src/lib.rs b/third_party/rust/cty/src/lib.rs
--- a/third_party/rust/cty/src/lib.rs	2023-06-05 12:01:58.166619411 +0800
+++ b/third_party/rust/cty/src/lib.rs	2023-06-05 12:02:42.050084151 +0800
@@ -45,6 +45,7 @@
